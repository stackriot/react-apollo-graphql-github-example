const fromEvent = require("graphcool-lib").frmEvent;
...
module.exports = function(event) {
	...
	function getGithubToken() {
		...
		}).then(data => data.json()).then(json => json.access_token);
	}
	function getGithubAccountData(githubToken) {
		if (!githubToken) {
			return Promise.reject("Github access_token is undefined.");
		}
		return fetch(`https://api.github.com/user?access_token=${githubToken}`).then(response => response.json()).then(parsedResponse => {
...
.then(userQueryResult => {
	...
	return getGithubToken().then(githubToken => {
		return getGithubAccountData(githubToken).then(githubUser => {
		...
		}).then(generateGraphcoolToken).then(token => {
			return {
				data: {
					token: token
				}
